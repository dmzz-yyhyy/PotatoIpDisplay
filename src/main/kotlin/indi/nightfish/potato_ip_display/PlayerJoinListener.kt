    ...
    fun onPlayerLogin(event: ServerPostConnectEvent) {
        event.player.sendMessage(
            Component.text("[").color(TextColor.color(85, 85, 85)).append(
                Component.text("PotatoIpDisplay").color(TextColor.color(255, 170, 0))
            ).append(
                Component.text("] ").color(TextColor.color(85, 85, 85))
            ).append(
                Component.text("您当前ip归属地 ").color(TextColor.color(255, 255, 85))
            ).append(
                Component.text("[").color(TextColor.color(170, 170, 170))
            ).append(
                Component.text("${IpATTRMap.playerIpATTRMap[event.player.username]}")
